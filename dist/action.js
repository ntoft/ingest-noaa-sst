...
// src/warmhub.ts
var DEFAULT_HOME_REPO = "fish-kill-attribution/noaa-sst-daily";
...
var ERDDAP_BASE = "https://coastwatch.pfeg.noaa.gov/erddap/tabledap";
var DATASET = "cwwcNDBCMet";
...
async function fetchObservations() {
  ...
  const query = `${vars}` + `&time>=${encodeURIComponent(start)}` + `&time<=${encodeURIComponent(end)}` + `&latitude>=${BBOX.minLat}&latitude<=${BBOX.maxLat}` + `&longitude>=${BBOX.minLon}&longitude<=${BBOX.maxLon}` + `&wtmp!=NaN`;
  ...
  if (res.status === 404)
    return [];
  if (!res.ok)
    throw new Error(`ERDDAP ${res.status}: ${await res.text()}`);
...
async function main() {
  ...
  const result = await client.commit.apply(orgName, repoName, `NOAA SST daily ingest ${today}`, operations);
